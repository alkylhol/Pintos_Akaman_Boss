...
list_elem   and returns the new file.  Returns a null pointer if an
...
/* Returns the file's directory
   Joshua drove here */
struct dir *get_dir (struct file *file) { return file->dir; }

/* Returns the file's inode's sector number
   Joshua drove here */
uint32_t get_sector_number (struct file *file)
{
  return inode_get_inumber (file->inode);
}

/* Returns whether the file's inode is a directory
   Joshua drove here */
uint32_t get_is_directory (struct file *file)
{
  return is_directory (file->inode);
}
